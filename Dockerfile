...
WORKDIR $GOPATH/src/github.com/Vedaad-Shakib/IoTeX-Sim/
...
RUN mkdir -p $GOPATH/src/github.com/CoderZhi/go-ethereum/ && \
    ...
    make clean build && \
    ln -s $GOPATH/src/github.com/Vedaad-Shakib/IoTeX-Sim/bin/server /usr/local/bin/iotex-server  && \
    ln -s $GOPATH/src/github.com/Vedaad-Shakib/IoTeX-Sim/bin/actioninjector /usr/local/bin/iotex-actioninjector && \
    ln -s $GOPATH/src/github.com/Vedaad-Shakib/IoTeX-Sim/bin/addrgen /usr/local/bin/iotex-addrgen && \
    cp ./crypto/lib/libsect283k1_ubuntu.so /usr/lib/ && \
    cp ./crypto/lib/blslib/libtblsmnt_ubuntu.so /usr/lib/ && \
    mkdir -p /etc/iotex/ && \
    ln -s $GOPATH/src/github.com/Vedaad-Shakib/IoTeX-Sim/blockchain/testnet_actions.yaml /etc/iotex/testnet_actions.yaml && \
